...
// humanBytes formats bytes into human-readable format (GB, MB, KB, B)
func humanBytes(v uint64) string {
	switch {
	case v >= 1<<40:
		return formatFloat(float64(v)/(1<<40)) + " TB"
	case v >= 1<<30:
		return formatFloat(float64(v)/(1<<30)) + " GB"
	case v >= 1<<20:
		return formatFloat(float64(v)/(1<<20)) + " MB"
	case v >= 1<<10:
		return formatFloat(float64(v)/(1<<10)) + " KB"
	default:
